...
const xpipe_1 = __importDefault(__nccwpck_require__(245));
const core = __importStar(__nccwpck_require__(519));
const server_1 = __nccwpck_require__(66);
async function main() {
    core.debug(`stat-helper daemon starting at ${new Date()}`);
    const serverSocketPath = xpipe_1.default.eq(path_1.default.join(core.locations.tempDir, `stat-helper-${process.pid}.sock`));
    if (process.send) {
        process.send(serverSocketPath);
    }
    core.debug(`listening on ${serverSocketPath}`);
    const server = await (0, server_1.listenEnv)(serverSocketPath);
...
const validate_1 = __nccwpck_require__(494);
;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.listenEnv = void 0;
const core = __importStar(__nccwpck_require__(519));
const net_1 = __importDefault(__nccwpck_require__(631));
const util_1 = __nccwpck_require__(669);
const not_a_log_1 = __importDefault(__nccwpck_require__(998));
const measurement_1 = __nccwpck_require__(730);
const impl_1 = __nccwpck_require__(792);
const validate_1 = __nccwpck_require__(494);
;
async function listen(serverSocket) {
    const server = net_1.default.createServer()
        .listen(serverSocket);
    let state = {};
    const handle = (0, impl_1.bindService)(server, {
        add: {
            validators: (0, validate_1.compile)({
                kind: 'tuple',
                types: [
                    { kind: 'guard', check: measurement_1.datapointValidator }
                ]
            }),
            async run(m) {
                if (m.id in state) {
        ...
        printReport: {
            ...
            async run() {
                ...
                for (const [k, m] of Object.entries(state)) {
                    report[k] = (0, measurement_1.formatMeasurement)(m);
                }
                return not_a_log_1.default.table(report);
            }
        },
        shutdown: {
            validators: (0, validate_1.compile)({
    ...
    await (0, util_1.promisify)(f => server.on('listening', f))();
    return handle;
}
async function listenEnv(serverSocket) {
...
const ts = new stream_1.Transform({ transform: (chunk, _, cb) => cb(null, chunk) });
const logger = new console_1.Console({ stdout: ts, stderr: ts, colorMode: false });
const handler = {
    get(_, prop) {
